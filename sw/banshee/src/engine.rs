...
impl Engine {
    ...
    unsafe fn execute_inner<'b>(&'b self) -> Result<u32> {
        ...
        debug!("Starting to allocate TCDM memories on {} clusters.", self.num_clusters);
...
impl<'a, 'b> Cpu<'a, 'b> {
    ...
    unsafe fn binary_trace(&self, addr: u32, inst: u32, accesses: &[TraceAccess], data: &[u64]) {
        ...
        let mut args = args.map(|(access, data)| match access {
            ...
            TraceAccess::ReadFReg(x) => format!("f{:02}:{:08x}", x, data as u64),
            TraceAccess::WriteFReg(x) => format!("f{:02}={:08x}", x, data as u64),
            // TraceAccess::ReadFReg(x) => format!("f{:02}:{:#b}", x, data as u64),
            // TraceAccess::WriteFReg(x) => format!("f{:02}={:#b}", x, data as u64),
