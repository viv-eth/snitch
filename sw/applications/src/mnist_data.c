...
uint8_t * get_labels(const char * path, uint32_t * number_of_labels)
{
    ...
    *number_of_labels = header.number_of_labels;
    ...
    if (compute_id == 0) {
        labels = snrt_l1alloc(*number_of_labels * sizeof(uint8_t));
    }
...
mnist_image_t * get_images(const char * path, uint32_t * number_of_images)
{
    ...
    *number_of_images = header.number_of_images;
    ...
    if (compute_id == 0) {
        images = snrt_l1alloc(*number_of_images * sizeof(mnist_image_t));
    }
...
mnist_dataset_t * mnist_get_dataset(const char * image_path, const char * label_path)
{
    ...
    if (compute_id == 0) {
        dataset = snrt_l1alloc(1 * sizeof(mnist_dataset_t));
    }
...
{
    mnist_dataset_t * chunk;

    int start_offset;

    start_offset = size * offset;
    ...
    chunk = calloc(1, sizeof(mnist_dataset_t));

    if (NULL == chunk) {
        return NULL;
    }

    chunk->images =  &dataset->images[start_offset];

    if (NULL == chunk->images) {
        mnist_free_dataset(chunk);
        return NULL;
    }

    chunk->labels = &dataset->labels[start_offset];

    if (NULL == chunk->labels) {
        mnist_free_dataset(chunk);
        return NULL;
    }
