...
void mnist_fp8(const network_fp8_t *n){
    ...
    // uint32_t bias_mat_size = n->OUT_CH * n->dtype; // NOTE: changing this to multiple of 8
        ...
        if (snrt_is_compute_core() && snrt_cluster_compute_core_idx() < compute_num && cluster_id == 1) {
            ...
            if(RUN_GRADIENT_UPDATE){
                ...
                if(BASELINE){
                    // INFO: baseline
                    benchmark_get_cycle();
                    gradient_update_fp8n(n->IN_CH1, n->IN_CH2, div, 
    ...
    if(setup_SSR && RUN_TRAINING_STEP){
        ...
        if(snrt_is_dm_core() && cluster_id==0) {
            ...
            snrt_dma_start_tracking();
            // WARN: make sure that pointer types are according to network precision
            char *weight_grad_ptr = ((uint32_t)weight_grads_cl0) + cluster_offset;
            char *bias_grad_ptr = ((uint32_t)biases_cl0) + cluster_offset;
    ...
    if (snrt_is_compute_core() && snrt_cluster_compute_core_idx() < compute_num && cluster_id == 0) {
        ...
        if(RUN_TRAINING_STEP){
            ...
            if(BASELINE){
                // INFO: baseline
                benchmark_get_cycle();
                training_step_fp8n(n->IN_CH1, n->IN_CH2, div, 
